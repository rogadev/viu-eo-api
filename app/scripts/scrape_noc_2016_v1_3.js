...
async function run() {
  ...
  await fs.writeFile(
    './temp/noc_2016_unit_group_links.json',
    JSON.stringify(unitLinks.flat())
  )

  // --------------------------------------- LINKS SCRAPED ---------------------------------------

  const nocLinks = require('./temp/noc_2016_unit_group_links.json')
  ...
  for (const link of nocLinks) {
    ...
    const obj = await page.evaluate(() => {
      ...
      for (const group of detailGroups) {
        ...
        // TODO: This worked fine with 2021 data, but 2016 has groups of ul's with headers, in some cases but not all.
